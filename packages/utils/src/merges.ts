...
export function deepMerge<T>(
  target: T,
  source: Partial<T>,
  getId: (data: any) => any = (data: any) => data.id,
): T {
  try {
    ...
    if (Array.isArray(target) && Array.isArray(source)) {
      const hasIds = source.every(
        (item) => isMergeableObject(item) && getId(item) !== undefined,
      );

      if (hasIds) {
        const targetMap = new Map(
          target.filter(isMergeableObject).map((item) => [getId(item), item]),
        );
    ...
    if (isMergeableObject(target) && isMergeableObject(source)) {
      for (const key in source) {
        ...
        const canMerge =
          targetValue &&
          (isMergeableObject(targetValue) || Array.isArray(targetValue)) &&
